...
from nets.yolo import get_train_model, Yolov6
from nets.loss import get_lr_scheduler, get_yolo_loss
...
if __name__ == "__main__":
    ...
    #   Unfreeze_batch_size     模型在解冻后的batch_size 569
    #------------------------------------------------------------------#
    UnFreeze_Epoch      = 569
    ...
    num_workers         = 1
    #-------------------------------------------------#
    # iou的类型 可选择的有 siou ciou 官网在使用nano和tiny版本时是ciou，s版本是siou
    iou_type            = 'siou'
    ...
    if ngpus_per_node > 1:
        with strategy.scope():
            ...
            #   创建yolo模型
            #------------------------------------------------------#
            model_body  = Yolov6([None, None, 3], num_classes=num_classes, phi=phi, weight_decay=weight_decay)
            if model_path != '':
                #------------------------------------------------------#
                #   载入预训练权重
                #------------------------------------------------------#
            ...
            if not eager:
                model       = get_train_model(model_body, input_shape, num_classes, iou_type=iou_type)
            else:
                yolo_loss   = get_yolo_loss(input_shape, len(model_body.output), num_classes, iou_type=iou_type)
    else:
        #------------------------------------------------------#
        #   创建yolo模型
        #------------------------------------------------------#
    ...
    show_config(
        ...
        save_period = save_period, save_dir = save_dir, num_workers = num_workers, num_train = num_train, num_val = num_val, iou_type=iou_type
    )
    #---------------------------------------------------------#
    #   总训练世代指的是遍历全部数据的总次数
    #   总训练步长指的是梯度下降的总次数 
